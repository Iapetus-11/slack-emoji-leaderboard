...
async def app_handle_reaction_removed(event: dict[str, Any]):
    ...
    if user_reactions := await SlackUserEmojiReactions.get_or_none(emoji_id=event['reaction'], user_id=event['user']):
...
async def api_emojis_leaderboards(since: datetime = Query(None)):
    ...
    data.update({
        r['emoji_id']: r['count']
        for r in
        await SlackUserEmojiReactions.all().values('emoji_id', 'count')
    })
...
async def sync_emojis():
    ...
    for name, url in response.data['emoji'].items():
        ...
        emojis[name], _ = await SlackEmoji.get_or_create(id=name, defaults={'hash': emoji_hash, 'url': url})

    # We treat emoji aliases as their own emojis in our db
    for alias_name, name in aliases.items():
        if emoji := emojis.get(name):
            await SlackEmoji.get_or_create(id=alias_name, defaults={'hash': emoji.hash, 'url': emoji.url})
...
if __name__ == "__main__":
    uvicorn.run('src.app:api', host=CONFIG.API_HOST, port=CONFIG.API_PORT, log_level=CONFIG.LOG_LEVEL.lower())
